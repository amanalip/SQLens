...
## 1. Help Dialog Keyboard Dismissal on Escape
...
## 2. Window Resize Diagram Auto-Fit
...
## 3. Query Diagnostics for Random Sorting & Redundant Distinct
...
## 4. TableNode Target Handle for CTE Ingress Connections
...
## 5. Diagnostics Bar Line 0 Rendering Bug
...
## 6. Results Table CSV Export Type Serialization
...
## 7. Concurrent SqlEngineClient Initialization Race Condition
...
## 8. Stale Callback Closure in CodeMirror Keymaps
...
## 9. URL Hash State Hydration Flash
...
## 10. Cursor Position Reset on SQL Formatter
...
## 11. Handle ID Collisions in React Flow Schema Cards
...
## 12. Schema Parser Multi-Word Data Types
...
## 13. Stale Error State on Database and Sample Selection
...
## 14. Theme Mismatch in Graph PNG Export
...
## 15. Resilient URL Hash State Decoding
...
## 16. SQLite Foreign Key Null Target Column Resolution
...
## 17. Web Worker SQLite Index Extraction & Missing FK Index Warnings
...
## 18. CTE Flow Edges in Query Graph
...
## 19. CTE Node Inspector in DetailsPanel
...
## 20. Zero-Column Query Feedback in Results Table
...
1. **Modal Keyboard Dismissal**: Pressing `Escape` closes the Help and Keyboard Shortcuts modal from anywhere in the app.
2. **Dynamic Canvas Auto-Centering**: Resizing the window or toggling full screen automatically triggers a smooth graph re-center.
3. **Advanced Performance Diagnostics**: Visual diagnostics warn about `ORDER BY RANDOM()` and redundant `DISTINCT` + `GROUP BY` patterns.
4. **Target Handle Docking on Table Nodes**: Visual connections from CTEs and subqueries dock directly into source table cards.
5. **Query Clear & Reset Button**: Dedicated Clear button in `EditorPane.tsx` to clear editor queries with one click.
6. **Global Escape Key Dismissal**: `Escape` key listener in `src/App.tsx` to dismiss open node inspection drawers.
7. **Cursor-Preserving Formatter**: Formatter maintains cursor and line position during keyword formatting.
8. **Interactive Table Filter in Schema Explorer**: Real-time table search input in `SchemaFlowCanvas.tsx` to highlight matching tables and dim non-matching tables.
9. **Download CSV & Page Size Controls**: "Download CSV" file export button and a page size selector (10, 25, 50, 100 rows per page) in `ResultsTable.tsx`.
10. **Active Sort Direction Indicators**: Ascending (`ArrowUp`), descending (`ArrowDown`), and unsorted (`ArrowUpDown`) icons in results table headers.
11. **Keyboard Shortcuts & Help Dialog**: Help modal in `TopNav.tsx` detailing key shortcuts (`Ctrl+Enter` to run, `Ctrl+Space` for autocomplete, node click inspection).
12. **Dedicated Sort & Limit Inspector**: Full inspection for `ORDER BY` directions and `LIMIT / OFFSET` numbers in `DetailsPanel.tsx`.

---

## Expanded Test Coverage (11 Test Suites, 45 Tests)
