...
## 1. Schema Mode Dynamic Masonry Table Placement
...
## 2. SQLite group_concat Dialect Misclassification
...
## 3. URL Hash State Type & Shape Guarding
...
## 4. Unique Key (UQ) Visual Badges on Table Cards
...
## 5. JOIN USING Column Reference Extraction Bug
...
## 6. JoinNode Empty usingColumns False-Positive Guard
...
## 7. ErrorBoundary Session Reset Recovery Option
...
## 8. CTESubgraphNode Target Handle for Chained Sub-Pipelines
...
## 9. Help Dialog Keyboard Dismissal on Escape
...
## 10. Window Resize Diagram Auto-Fit
...
## 11. Query Diagnostics for Random Sorting & Redundant Distinct
...
## 12. TableNode Target Handle for CTE Ingress Connections
...
## 13. Diagnostics Bar Line 0 Rendering Bug
...
## 14. Results Table CSV Export Type Serialization
...
## 15. Concurrent SqlEngineClient Initialization Race Condition
...
## 16. Stale Callback Closure in CodeMirror Keymaps
...
## 17. URL Hash State Hydration Flash
...
## 18. Cursor Position Reset on SQL Formatter
...
## 19. Handle ID Collisions in React Flow Schema Cards
...
## 20. Schema Parser Multi-Word Data Types
...
## 21. Stale Error State on Database and Sample Selection
...
## 22. Theme Mismatch in Graph PNG Export
...
## 23. Resilient URL Hash State Decoding
...
## 24. SQLite Foreign Key Null Target Column Resolution
...
## 25. Web Worker SQLite Index Extraction & Missing FK Index Warnings
...
## 26. CTE Flow Edges in Query Graph
...
## 27. CTE Node Inspector in DetailsPanel
...
## 28. Zero-Column Query Feedback in Results Table
...
1. **Overlap-Free Masonry Schema Layout**: Table cards in Schema Mode are placed in dynamic vertical columns that prevent tall tables from colliding.
2. **Visual Unique Constraint Badges (UQ)**: Schema cards highlight unique keys with cyan `UQ` badges.
3. **Session Reset Option in Error Boundary**: Recovery button allows users to reset broken state or corrupted hash parameters.
4. **Chained CTE Ingress Ports**: Visual connection ports allow chaining between sub-pipelines.
5. **Modal Keyboard Dismissal**: Pressing `Escape` closes the Help and Keyboard Shortcuts modal from anywhere in the app.
6. **Dynamic Canvas Auto-Centering**: Resizing the window or toggling full screen automatically triggers a smooth graph re-center.
7. **Advanced Performance Diagnostics**: Visual diagnostics warn about `ORDER BY RANDOM()` and redundant `DISTINCT` + `GROUP BY` patterns.
8. **Target Handle Docking on Table Nodes**: Visual connections from CTEs and subqueries dock directly into source table cards.
9. **Query Clear & Reset Button**: Dedicated Clear button in `EditorPane.tsx` to clear editor queries with one click.
10. **Global Escape Key Dismissal**: `Escape` key listener in `src/App.tsx` to dismiss open node inspection drawers.
11. **Cursor-Preserving Formatter**: Formatter maintains cursor and line position during keyword formatting.
12. **Interactive Table Filter in Schema Explorer**: Real-time table search input in `SchemaFlowCanvas.tsx` to highlight matching tables and dim non-matching tables.
13. **Download CSV & Page Size Controls**: "Download CSV" file export button and a page size selector (10, 25, 50, 100 rows per page) in `ResultsTable.tsx`.
14. **Active Sort Direction Indicators**: Ascending (`ArrowUp`), descending (`ArrowDown`), and unsorted (`ArrowUpDown`) icons in results table headers.
15. **Keyboard Shortcuts & Help Dialog**: Help modal in `TopNav.tsx` detailing key shortcuts (`Ctrl+Enter` to run, `Ctrl+Space` for autocomplete, node click inspection).
16. **Dedicated Sort & Limit Inspector**: Full inspection for `ORDER BY` directions and `LIMIT / OFFSET` numbers in `DetailsPanel.tsx`.
